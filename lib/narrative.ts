...
const OPENING: Record<VehicleCategory, (v: Vehicle) => string> = {
  Hatchback: (v) =>
    `Un ${v.marca} ${v.modelo} pensado para el uso diario: ágil en la ciudad, fácil de estacionar y con el consumo que pide la rutina de Punta del Este.`,
  Sedán: (v) =>
  ...
  SUV: (v) =>
    `Un ${v.marca} ${v.modelo} con la altura y la versatilidad que pide la rutina de Punta del Este, dentro y fuera del asfalto.`,
  Utilitario: (v) =>
...
export function narrativeFor(vehicle: Vehicle): string[] {
  ...
  const facts = `Año ${vehicle.anio}, con ${fmtInt(vehicle.km)} km documentados. Motor a ${vehicle.combustible.toLowerCase()}, caja ${vehicle.transmision.toLowerCase()}. Ingresó a nuestra selección el ${fmtIngreso(vehicle.ingreso)} y, como toda unidad de Gonzalo Villasuso, pasó por una revisión mecánica completa antes de publicarse.`;
